...
import { getRuntimeSettings, getAestheticDna } from "./storageService.js";
...
export async function fetchNextRadioSegment() {
    ...
    try {
        ...
        const playlistResp = await streamDJ.generatePlaylist(`候选列表: [${candidateList}]。请选一首契合 DNA 的信号。`, "zh-CN", "classic", currentThemeContext || undefined, 1);
        const selectedSong = playlistResp.songs[0];
        let targetTrack = finalCandidates.find(t => t.title.toLowerCase().includes(selectedSong.title.toLowerCase()) ||
        ...
        const narrationResp = await streamDJ.generateNarrationForTrack({ ...targetTrack, releaseYear: wiki?.releaseYear, trivia: wiki?.trivia?.[0] }, playlistResp.theme_update, "zh-CN", "night");
        const dj_talk = narrationResp.dj_talk;
        currentThemeContext = narrationResp.theme_update;
        const ttsResult = await synthesizeSpeech(dj_talk, settings.defaultVoice, {
