...
async function broadcastNarration(text) {
    ...
    if (ttsResult.audioBase64) {
        ...
        const isChatMode = agentPlayer.isLooping && (playerState.current?.title.includes("Beautiful Lady") || playerState.queue.some(q => q.title.includes("Beautiful Lady")));
        if (isChatMode) {
            let root = process.cwd();
            while (root !== "/" && !fs.existsSync(path.join(root, ".git"))) {
    ...
    try {
        ...
        if (msg === '/talk' || msg === '/chat') {
            ...
            if (ttsResult.audioBase64) {
                ...
                agentPlayer.add(bgmPath, "Beautiful Lady - Daydream");
                agentPlayer.isLooping = true; // 保持循环
            }
            return res.json({
                logs: [
