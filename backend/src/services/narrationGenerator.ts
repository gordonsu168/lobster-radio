...
// 语言映射表
const LANGUAGE_PHRASES: Record<DJLanguage, typeof DJ_PHRASES_HK> = {
  "zh-HK": DJ_PHRASES_HK,
  "zh-CN": DJ_PHRASES_CN,
  "en-US": DJ_PHRASES_EN,
};
...
let currentLanguage: DJLanguage = "zh-CN";

// 设置 DJ 语言
export function setDJLanguage(lang: DJLanguage) {
...
// 获取歌曲名称连接词（不同语言有不同的）
function getSongConnector(): string {
  const connectors: Record<DJLanguage, string> = {
    "zh-HK": "嘅",
    "zh-CN": "的",
    "en-US": " by ",
  };
...
// 主函数：根据风格生成旁白
export function generateNarration(song: SongWiki, style: DJStyle = "classic", language?: DJLanguage): string {
  // 如果指定了语言，临时切换
...
// 随机选择风格
export function generateRandomNarration(song: SongWiki, language?: DJLanguage): string {
