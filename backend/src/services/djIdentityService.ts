...
export function getDJIdentity(): DJIdentity {
  try {
...
export function saveDJIdentity(identity: DJIdentity): void {
...
export function getDJPersona(): string {
  try {
...
export function saveDJPersona(content: string): void {
  writeFileSync(PERSONA_FILE, content, "utf-8");
}

function parseIdentityMarkdown(content: string): DJIdentity {
  const identity: Partial<DJIdentity> = {};
  
  const mappings: Record<string, keyof DJIdentity> = {
    "Name": "name",
