...
import type { Preferences, RuntimeSettings, PlaybackHistoryItem } from "../types.js";
...
export function addPlayHistory(item: PlaybackHistoryItem) {
  initDb();
  ...
  `);
  stmt.run(
    item.id, item.title, item.artist, item.album, item.previewUrl, item.artwork,
    JSON.stringify(item.moodTags || []),
    item.energy, item.explanation, item.source, item.playedAt, item.feedback || null
  );
}

export function updateFeedback(trackId: string, feedback: "like" | "dislike") {
  initDb();
  const stmt = db.prepare(`
...
export function getPlayHistory(limit = 50): PlaybackHistoryItem[] {
  initDb();
...
export function addChatHistory(role: "user" | "assistant", content: string) {
  initDb();
  const stmt = db.prepare(`INSERT INTO chat_history (role, content) VALUES (?, ?)`);
  stmt.run(role, content);
}

export function getChatHistory(limit = 50): { role: "user" | "assistant"; content: string }[] {
  initDb();
...
export async function getPreferences(): Promise<Preferences> {
  ...
  const prefs = await readJson<Preferences>(preferencesPath, defaultPreferences);
...
export async function savePreferences(preferences: Preferences) {
  ...
  delete (toSave as any).history;
  delete (toSave as any).chatHistory;
