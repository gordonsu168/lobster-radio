...
import { scanMusicLibrary, getAudioStream, getLibraryStats } from "../services/musicLibraryService.js";
...
// 重新扫描音乐库
libraryRouter.post("/scan", async (_req: Request, res: Response) => {
  try {
...
// 流式播放音频文件
libraryRouter.get("/stream/:filePathBase64", async (req: Request, res: Response) => {
  try {
    const fs = await import("node:fs");
    const filePathBase64 = req.params.filePathBase64 as string;
