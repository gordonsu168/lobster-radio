...
export class RadioDJAgent {
    ...
    // Generate intro
    async generateIntro(song, mood, style, language, contextSummary, memoryInsight) {
    ...
    // Generate outro
    async generateOutro(song, style, language, contextSummary, memoryInsight) {
        const model = createOptionalModel();
        if (!model) {
    ...
    async generateMidTrackTrivia(song, style, language, preStoredFact) {
        const model = createOptionalModel();
        if (!model) {
    ...
    // Intro user prompt
    getIntroUserPrompt(song, mood, contextSummary, memoryInsight, language) {
        const basePrompts = {
    ...
    // Outro user prompt
    getOutroUserPrompt(song, contextSummary, memoryInsight, language) {
        const basePrompts = {
    ...
    // Trivia user prompt
    getTriviaUserPrompt(song, language, preStoredFact) {
    ...
    getFallbackIntro(song, style, language) {
        const templates = {
            "zh-CN": {
                classic: [
    ...
    getFallbackOutro(song, style, language) {
        const templates = {
            "zh-CN": {
                classic: [
    ...
    getFallbackTrivia(song, language) {
        const templates = {
            "zh-CN": [
