...
export class LobsterCoreXAgent {
  private dna: AestheticDNA;
  private logs: XPacket[] = [];
  private model = createOptionalModel();

  constructor(initialDna?: AestheticDNA) {
    this.dna = initialDna || this.getInitialDna();
  }

  private getInitialDna(): AestheticDNA {
    return {
  ...
  private addLog(type: XPacketType, content: string, metadata?: any): XPacket {
   ...
   * 支持思维链和自主决策
   */
  async chat(message: string): Promise<XPacket[]> {
    this.logs = [];
    ...
    try {
      ...
      this.addLog('message', response.content as string);
    } catch (e) {
  ...
  async stealthSniff(localTracks: Track[], ncmHistory: PlaybackHistoryItem[]): Promise<XPacket[]> {
  ...
  async ghostDig(seedTrack: string): Promise<Track[]> {
