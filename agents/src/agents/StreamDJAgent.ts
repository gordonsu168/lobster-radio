...
export class StreamDJAgent {
  private identity: DJIdentity;

  constructor(identity: DJIdentity = DEFAULT_DJ_IDENTITY) {
  ...
  private getSystemPrompt(style: DJStyle, language: DJLanguage, themeContext?: ThemeContext): string {
...
The response MUST be valid JSON:
{
  ...
  async generateNextSegment(
    ...
    if (lastSong) {
      ...
      if (lastSong.album) userPrompt += `，收录于专辑《${lastSong.album}》`;
      if (lastSong.explanation) userPrompt += `。简介: ${lastSong.explanation}`;
  ...
  private parseJsonResponse(contentStr: string): any {
    const jsonMatch = contentStr.match(/```(?:json)?\s*(\{[\s\S]*?\})\s*```/);
  ...
  async generatePlaylist(
    ...
    const fallback: PlaylistResponse = {
      theme_update: {
        theme: themeContext?.theme || "音乐漫游",
        phase: "intro",
        coveredTopics: []
    ...
    const prompts: Record<DJLanguage, string> = {
...
Return valid JSON:
{
  ...
  "intro_talk": "opening remarks..."
    ...
    try {
      ...
      if (parsed.theme_update && Array.isArray(parsed.songs) && parsed.songs.length > 0 && parsed.intro_talk) {
        return parsed as PlaylistResponse;
      }
      return fallback;
    } catch (e) {
  ...
  async generateNarrationForTrack(
    track: TrackInfo,
    ...
    const fallback: NarrationResponse = {
...
`
    ...
    try {
      ...
      if (parsed.dj_talk) {
        return parsed as NarrationResponse;
      }
      return fallback;
    } catch (e) {
